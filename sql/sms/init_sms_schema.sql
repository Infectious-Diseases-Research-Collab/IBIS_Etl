...
-- status) lives in message_status_view.sql, not here: it joins ibis.baseline,
-- which doesn't exist until promote_ibis has run at least once, so db.py's
-- init_sms_tables() creates it separately and conditionally.
