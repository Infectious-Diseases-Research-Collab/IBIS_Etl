...
class PromoteIbis(BaseStage):
    ...
    def run(self) -> StageResult:
        ...
        # The inner raise (in _promote_table) aborts the transaction on the
        ...
        try:
            with self.engine.begin() as conn:
                ...
                for table in tables:
                    try:
                        self._promote_table(conn, table)
    ...
    def _promote_table(self, conn, table: str) -> None:
        """Blue-green swap gold_ibis.<table> into ibis.<table>, preserving
        dependent view definitions across the rename."""
