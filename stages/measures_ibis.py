...
class MeasuresIbis(BaseStage):
    ...
    def run(self) -> StageResult:
        ...
        baseline_had_data = not silver_df.empty

        if not baseline_had_data:
            logger.warning("silver_ibis.baseline is empty — skipping baseline measures.")
        else:
        ...
        # Followup is validated independently of baseline — an empty
        # baseline (e.g. all MDBs quarantined that day) must not prevent
        # followup from being checked, since the two tables can diverge.
        followup_had_data = self._validate_followup(all_reports, errors)

        if not baseline_had_data and not followup_had_data:
            logger.warning("silver_ibis.baseline and silver_ibis.followup are both empty — skipping measures.")
    ...
    def _validate_followup(self, all_reports: list[pd.DataFrame], errors: list[str]) -> bool:
        """
        ...
        Returns True if silver_ibis.followup had any rows (regardless of
        whether the check found issues, found none, or a per-site call
        failed), False only when it was confirmed empty. run() uses this —
        together with whether baseline had data — to distinguish "there was
        genuinely nothing to validate" (benign) from "validation ran but
        produced zero reports because every attempt failed" (a real
        failure), now that baseline being empty no longer skips this method.
        """
        try:
            ...
            for country, country_group in followup_df.groupby('country'):
                ...
                for site, group in site_groups:
                    ...
                    try:
                        ...
                        # Append unconditionally (even when empty of issues) to
                        # match the baseline loop's convention — all_reports
                        # being non-empty is how run() tells "something ran
                        # successfully" apart from "every attempt failed".
                        all_reports.append(report)
                    except Exception as exc:
